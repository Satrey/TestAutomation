# Реализована возможность изменения количества товара добавляемого в корзину
# путем изменения значения переменной 'cart_amount' максимальное значение 6.
    ...
    for i, item in enumerate(items, start=1):
        if i > cart_amount:
            ...
            if text == 'Add to cart':
                ...
                print(f'Добавили товар {product_name.text} с ценой {product_price.text} в корзину')
                item_list[product_name.text] = product_price.text.strip('$')
            else:
                item_list[product_name.text] = product_price.text.strip('$')
                print(f'Добавили товар {product_name.text} с ценой {product_price.text} в список')                  
    return item_list
...
with webdriver.Chrome(options=options) as browser:
    ...
    assert browser.current_url == 'https://www.saucedemo.com/inventory.html', 'Неудача!!! Переход на главную страницу магазина не осуществлен!'
    print(f'Успешно!!! Переход на главную страницу выполнен по адресу {browser.current_url}!')
    ...
    assert cart_amount == int(cart_bage.text), 'Ошибка! Количество товара не совпадает'
    ...
    assert product_price_add_cart == product_price_in_cart == product_price_in_summary,\
    ...
    assert total == subtotal, 'Ошибка!!! Общая стоимость товаров без комиссии не совпадает!'
    ...
    assert total + tax == summary_total, 'Ошибка!!! Общая стоимость товаров включая комиссию не совпадает!'
