...
with webdriver.Chrome(options=options) as browser:
    ...
    print('\n************************************************')
    print(f'Начало теста основного функционала магазина  {base_url} {datetime.datetime.now().\
                                                                     time().strftime('%H:%M:%S')} !!!')
    print('************************************************\n')
    ...
    assert len(product_add_cart) == int(cart_bage.text), 'Ошибка! Количество товара не совпадает'
    ...
    assert round((total + tax), 2) == summary_total, 'Ошибка!!! Общая стоимость товаров включая комиссию не совпадает!'
